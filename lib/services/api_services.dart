...
import 'package:movieapp_checkpoint4/models/movie_detail_model.dart';
...
class ApiServices {
  ...
  Future<ResultsActor> getActorsList() async {
    var endPoint = 'person/popular';
    final url = '$baseUrl$endPoint$key';

    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      return ResultsActor.fromJson(jsonDecode(response.body));
    }
    throw Exception('failed to load actors list');
  }
  ...
  Future<ActorDetailModel> getActorDetail(int personId) async {
    final endPoint = 'person/$personId';
    final url = '$baseUrl$endPoint$key';

    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      return ActorDetailModel.fromJson(jsonDecode(response.body));
    }
